...
//                String retrunMsg = HttpUtil.httpXml(redisUtil.hget("COM:CIRCLE", "atomosb") + "/HTKG/SuperOSBEntry", messageSql, 30);
    ...
    public Future<BankAcctId> getBankAcctId(String acctId, String agentId) {
        try {
            SysMessage sysMessage = sysMessageMapper.selectOne(new QueryWrapper<SysMessage>()
                    .eq("type", "BankAcctIdInfor")
                    .eq("type_source", "ATOMOSB")
                    .eq("message_type", "SQL"));
            String messageSql = sysMessage.getMessage()
                    .replaceAll("#AGENTID#", agentId)
                    .replaceAll("#ACCTID#", acctId);
            log.info("拼装完成银行账户信息SQL[{}]", messageSql);
            String retrunMsg = atomosb.getData(messageSql);
            log.info("执行银行账户信息返回[{}]", retrunMsg);
            List<BankAcctId> bankAcctIds = JSON.parseArray(retrunMsg, BankAcctId.class);
            if (bankAcctIds != null && bankAcctIds.size() > 0){
                return new AsyncResult<BankAcctId>(bankAcctIds.get(0));
            }else {
                return null;
            }
        } catch (Exception e) {
            log.error("获取银行账户信息异常", e);
            return null;
        }
    }
    ...
    public void cacheBankAcctId(List acctList, String agentId) {
        for (Object ob : acctList) {
            redisUtil.lSet("AOSB:" + agentId + ":ACC", ob, TimeUnit.MINUTES.toSeconds(2), true);
        }
